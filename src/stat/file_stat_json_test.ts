...
Deno.test("exportStatJson()", async (t) => {
  const exportPath = "./fixtures/stat.json";
  const paths = ["fixtures/text/sample.txt"];

  await t.step("Create a sample file", async () => {
    await Deno.writeTextFile("fixtures/text/sample.txt", "hello");
  });
  ...
  await t.step("exportStatJson() second time", async () => {
    assertEquals(
      await Deno.readTextFile(exportPath),
      '{"fixtures/text/sample.txt":{"bytes":[5]}}',
    );

    // Modify the sample file.
    await Deno.writeTextFile("fixtures/text/sample.txt", "hello, world");
    await exportStatJson(exportPath, paths);

    assertEquals(
      await Deno.readTextFile(exportPath),
      '{"fixtures/text/sample.txt":{"bytes":[5,12]}}',
    );
  });
